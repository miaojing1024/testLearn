      ...
      <div class="appBody">
        ...
        <div class="appBodyDetails">
          <div class="details">
              ...
              <Showorno class="inner_details" :data="formateTime(detail.createTime)"/>
          </div>
          <div class="details">
              <div class="item_details">联系人</div>
              <Showorno class="inner_details" :data="detail.leagueConntracts || EMPTY_VAL_TEXT"/>
          </div>
          <div class="details">
              <div class="item_details">联系电话</div>
              <Showorno class="inner_details" :data="detail.leaguePhone || EMPTY_VAL_TEXT" />
          </div>
          <div class="details">
              <div class="item_details">申请说明</div>
              <Showorno class="inner_details am-text-break" :data="detail.applyReason || EMPTY_VAL_TEXT" maxLen="50"/>
          </div>
        </div>
      </div>
  </div>
...
export default {
  ...
  async mounted() {
    if (this.no && this.companyNo) {
      const {result} = await http.hyshdetail({no:this.no,companyNo:this.companyNo});
      console.log(result);
      this.detail = result
    }
  },
